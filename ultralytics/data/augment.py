...
class UnderwaterColorRandomTransfer:
    """
    Underwater Color Random Transfer (UCRT).

    A color augmentation tailored for underwater imagery, implemented by perturbing HSV channels while keeping
    hue shifts consistent with a typical underwater hue interval.

    Expected input image format: BGR uint8 (as used throughout Ultralytics dataloading/augment pipeline).
    """

    underwater_hue_interval = (18, 116)  # from UnitModule paper/code (Hue in [0, 180))
    ...
    def __call__(self, labels):
        ...
        hue_able = self._rand_bool(self.hue_prob)
        sat_able = self._rand_bool(self.saturation_prob)
        val_able = self._rand_bool(self.value_prob)
        if not any((hue_able, sat_able, val_able)):
            return labels
        ...
        if sat_able:
            sat_gain = self._rand_mult() * self.saturation_delta
            img_hsv[..., 1] = np.clip(img_hsv[..., 1] + sat_gain, 0, 255)
        if val_able:
            val_gain = self._rand_mult() * self.value_delta
            img_hsv[..., 2] = np.clip(img_hsv[..., 2] + val_gain, 0, 255)
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    color_aug = (
        UnderwaterColorRandomTransfer(
            hue_prob=getattr(hyp, "ucrt_hue_prob", 0.5),
            saturation_prob=getattr(hyp, "ucrt_saturation_prob", 0.5),
            value_prob=getattr(hyp, "ucrt_value_prob", 0.5),
            hue_delta=getattr(hyp, "ucrt_hue_delta", 5),
            saturation_delta=getattr(hyp, "ucrt_saturation_delta", 30),
            value_delta=getattr(hyp, "ucrt_value_delta", 30),
        )
        if getattr(hyp, "ucrt", False)
        else RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v)
    )
