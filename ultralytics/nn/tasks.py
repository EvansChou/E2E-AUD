...
class DetectionModel(BaseModel):
    ...
    def loss(self, batch, preds=None):
        """
        Compute detection loss, optionally adding UnitModule unsupervised losses for joint training.

        Notes:
            - UnitModule/UnitEnhance are expected to be placed before the detector in the model graph.
            - UnitModule stores per-forward auxiliary losses in `unit_losses` during training.
        """
        if getattr(self, "criterion", None) is None:
            self.criterion = self.init_criterion()
        ...
        # Add UnitModule losses if present.
        for m in self.model.modules():
            unit_total = getattr(m, "unit_loss", None)
            if unit_total is not None:
                det_loss = det_loss + unit_total
                # Clear non-leaf tensors to avoid deepcopy errors in EMA/checkpointing.
                m.unit_loss = None
                break
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[0]]
        elif m in {UnitEnhance, UnitModule}:
            # Image enhancement module keeps channel dimension (expects RGB input).
            c2 = ch[f]
            args = [c2, *args]  # prepend c1
        else:
            c2 = ch[f]
