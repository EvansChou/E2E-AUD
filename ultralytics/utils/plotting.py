...
def plot_results(file="path/to/results.csv", dir="", segment=False, pose=False, classify=False, on_plot=None):
    ...
    # Determine subplot layout from the first results file (so we can optionally include UnitModule losses).
    unit_keys = ["unit/loss_t", "unit/loss_acc", "unit/loss_cc", "unit/loss_sp", "unit/loss_tv"]
    unit_index = []
    try:
        _data0 = pd.read_csv(files[0])
        _cols0 = [x.strip() for x in _data0.columns]
        unit_index = [_cols0.index(k) for k in unit_keys if k in _cols0]
    except Exception:
        unit_index = []
    ...
    if classify:
        ...
        base_index = [2, 3, 4, 5, 6, 9, 10, 11, 7, 8]
        if unit_index:
            # Add up to 5 unit losses as an extra row in results.png.
            fig, ax = plt.subplots(3, 5, figsize=(12, 9), tight_layout=True)
            index = base_index + unit_index[:5]
        else:
